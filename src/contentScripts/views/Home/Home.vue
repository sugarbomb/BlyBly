...
function computeTabs(): HomeTab[] {
  ...
  const visibilityByPage = new Map(settings.value.homePageTabVisibilityList.map(t => [t.page, t.visible]))
  ...
  function addTab(page: HomeSubPage, target: HomeTab[]) {
    if (!visibilityByPage.get(page))
      return

    const tabItem: HomeTab = {
      i18nKey: tabI18nKeyByPage.get(page) || page,
      page,
    }
  ...
  // Explicitly control ordering (not affected by settings/homeTabs ordering).
  addTab(HomeSubPage.Ranking, targetTabsRankTrending)
  addTab(HomeSubPage.Trending, targetTabsRankTrending)
  addTab(HomeSubPage.Following, targetTabsFollowLive)
  addTab(HomeSubPage.Live, targetTabsFollowLive)
  addTab(HomeSubPage.SubscribedSeries, targetTabsSubForYou)
  addTab(HomeSubPage.ForYou, targetTabsSubForYou)
  ...
  if (!targetTabs.some(t => t.page === activatedPage.value) && targetTabs.length) {
    const forYouTab = targetTabs.find(t => t.page === HomeSubPage.ForYou)
    activatedPage.value = forYouTab?.page ?? targetTabs[0].page
  }
...
onMounted(() => {
  ...
  if (allTabs.value.length) {
    const forYouTab = allTabs.value.find(t => t.page === HomeSubPage.ForYou)
    activatedPage.value = forYouTab?.page ?? allTabs.value[0].page
  }
})
